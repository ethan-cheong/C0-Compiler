...
(* Skip entire regalloc phase (including liveness analysis) if more than 
   [_skip_regalloc] temps are assigned. 
   As of lab2, this number is induced by beorn-ret_negation.l1
   *)
let _skip_regalloc = 2000
...
let compile (cmd : cmd_line_args) : unit =
  (* Run the L1 checkpoint and do nothing else *)
  (* say_if cmd.regalloc_only (fun () -> "Regalloc checkpoint with file..." ^ cmd.filename);
  ...
  Typechecker.typecheck elaborated_ast;
  ...
  let ir = Trans.translate elaborated_ast in
  ...
  say_if cmd.dump_all (fun () -> "\n-----------3-address assembly-------------\n");
  say_if cmd.dump_all (fun () ->
    List.fold (List.map three_addr_assem ~f:Threeaddrassem.format) ~init:"" ~f:(fun x y ->
      x ^ "\n" ^ y));
  (* Instruction Selection *)
  let abstr_assem = Instrsel.instr_sel three_addr_assem in
  say_if cmd.dump_all (fun () -> "\n-----------Assem (after instr sel)-------------\n");
  say_if cmd.dump_all (fun () ->
    List.fold (List.map abstr_assem ~f:Assem.format) ~init:"" ~f:(fun x y -> x ^ "\n" ^ y));
  ...
  let abstr_assem_coloured =
    if temp_counter > _skip_regalloc
    then (
      say_if cmd.dump_all (fun () ->
        sprintf "\nNum temps: %i; skipping regalloc\n" temp_counter);
      abstr_assem)
    else (
      (* say_if cmd.dump_all (fun () -> "\n-----------Liveness-------------\n"); *)
      let liveness_info = Liveness.liveness abstr_assem in
      (* say_if cmd.dump_all (fun () -> Liveness.string_of_op_lst liveness_info); *)
      let intf_graph = Buildintfgraph.build_interference_graph liveness_info in
      let node_ordering =
        if temp_counter > _skip_mcs
        then Hashtbl.keys intf_graph
        else Maxcardsearch.max_cardinality_search intf_graph
      in
      Replacetemps.replace_temps
        abstr_assem
        (Greedycolouring.greedy_colouring ~debug:false intf_graph node_ordering))
  in
  say_if cmd.dump_all (fun () -> "\n-----------Assem (After Regalloc)----------\n");
  say_if cmd.dump_all (fun () ->
    List.fold (List.map abstr_assem_coloured ~f:Assem.format) ~init:"" ~f:(fun x y ->
      x ^ "\n" ^ y));
  (* Remove temps *)
  say_if cmd.dump_all (fun () -> "\n-----------Assem (After Stack Spill)----------\n");
  let abstr_assem_no_temps = Stackspill.stack_spill abstr_assem_coloured in
  say_if cmd.dump_all (fun () ->
    List.fold (List.map abstr_assem_no_temps ~f:Assem.format) ~init:"" ~f:(fun x y ->
      x ^ "\n" ^ y));
  (* Instr Sel *)
  say_if cmd.dump_all (fun () -> "\n------------------x86----------------\n");
  let x86_assem = Codegen.code_gen abstr_assem_no_temps in
  say_if cmd.dump_assem (fun () -> List.to_string ~f:Finalassem.format x86_assem);
  say_if cmd.dump_all (fun () ->
    List.fold (List.map x86_assem ~f:Finalassem.format) ~init:"" ~f:(fun x y ->
      x ^ "\n" ^ y));
  match cmd.emit with
  ...
  | Abstract_assem ->
    ...
    Out_channel.with_file file ~f:(fun out ->
      ...
      List.iter ~f:output_instr abstr_assem;
  ...
  | X86_64 ->
    ...
    Out_channel.with_file file ~f:(fun out ->
      let output_instr instr = Out_channel.fprintf out "%s\n" (Finalassem.format instr) in
      output_instr (Finalassem.Directive "_c0_main:");
      output_instr (Finalassem.Directive (".file\t\"" ^ cmd.filename ^ "\""));
      output_instr (Finalassem.Directive ".globl \"_c0_main\"");
      List.iter ~f:output_instr x86_assem)
;;

let run (cmd : cmd_line_args) : unit =
  try
    if cmd.regalloc_only
    then failwith "regalloc cmd" (* change this back to regalloc *)
    else compile cmd
  with
  | Error_msg.Error ->
