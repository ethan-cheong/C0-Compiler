...
let node_from_int x = Node.node_of_operand (AS.Temp (Temp.make x))
let node_from_reg x = Node.node_of_operand (AS.Reg x)
...
let create_adjlist_test =
  ...
  let rx = node_from_reg AS.R10D in
  let ry = node_from_reg AS.R11D in
  let rz = node_from_reg AS.R12D in
  let rax = node_from_reg AS.EAX in
