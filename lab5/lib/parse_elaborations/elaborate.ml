...
let is_mem_access lhs =
  ...
  match lhs_data with
  | Ast.Var _ -> false
  | Ast.Deref _ | Ast.Array_access _ | Ast.Struct_access_parse _ -> true
...
and process h =
  ...
  | Ast.Assign (e1, e2) ->
    mark_with_span (Ast.Assign (extract_paren e1, extract_paren e2)) stm_src
  | Ast.Return e -> mark_with_span (Ast.Return (extract_paren e)) stm_src
  | Ast.Return_void -> h
  | Ast.Nop -> h
  | Ast.Expr_stm e -> mark_with_span (Ast.Expr_stm (extract_paren e)) stm_src
  | Ast.Block_intermediate lst -> expand_ast_list lst
  | Ast.If (mexp, s1, s2) ->
    mark_with_span (Ast.If (extract_paren mexp, process s1, process s2)) stm_src
  | Ast.Seq (s1, s2) -> mark_with_span (Ast.Seq (process s1, process s2)) stm_src
  | Ast.While (e, s) -> mark_with_span (Ast.While (extract_paren e, process s)) stm_src
  | Ast.For (s1, s2, e, s3) ->
    mark_with_span (expand_for s1 s2 (extract_paren e) s3) stm_src
  | Ast.Declare _ -> h
  | Ast.Assert e -> mark_with_span (Ast.Assert (extract_paren e)) stm_src
  | Asnop_pure_mem _ -> h
  | Asnop_impure_mem _ -> h
  (* Still required as the ops are options right now *)
  | Asnop_pure_mem_intermediate { lhs; op; rhs } ->
    (match op with
     | None -> mark_with_span (Ast.Assign (extract_paren lhs, extract_paren rhs)) stm_src
     | Some x ->
       mark_with_span
         (Ast.Asnop_pure_mem { lhs = extract_paren lhs; op = x; rhs = extract_paren rhs })
         stm_src)
  | Asnop_impure_mem_intermediate { lhs; op; rhs } ->
    (match op with
     ...
     | Some x ->
       mark_with_span
         (Ast.Asnop_impure_mem
            { lhs = extract_paren lhs; op = x; rhs = extract_paren rhs })
         stm_src)
...
and process_decl decl tau tail =
  ...
  | Ast.Init (s, e) ->
    let new_init = Ast.Init (s, extract_paren e) in
    Ast.Declare (new_init, tau, expand_ast_list tail)
