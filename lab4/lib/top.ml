...
let _skip_regalloc_l1_l2 = 936 (* Induced by robert-py-regalpar-2-party-harder-small.l2 *)
...
(* Skip entire regalloc phase (including liveness analysis) if more than 
     ...
     *)
(* let _skip_regalloc_l3 = 5181 *)

let _skip_regalloc_l3 = 1749 (* novial-success_i_bet_our_code_will_fail_this.l4 *)

(* let _skip_regalloc_l4 = 1749 *)
...
(* Skip regalloc for a function (including liveness analysis) if more than 
     ...
     body.
     As of lab3, this is induced by NOTHING!! Tune this before submitting l4
     *)
let _skip_regalloc_per_function = 10000
...
let compile (cmd : cmd_line_args) : unit =
  ...
  let ir = Trans.translate typechecked_elab_ast struct_to_size in
  ...
  (* 
    (* Removing unused function args*)
    if cmd.dump_all then mark_timestamp timestamp_table "filterargs";
    let three_addr_assem_filtered = Filterargs.filter_args three_addr_assem in
    if cmd.dump_all then mark_timestamp timestamp_table "filterargs";
    say_if cmd.dump_all (fun () ->
      "\n------3-address assembly (after filtering args)------\n");
    say_if cmd.dump_all (fun () -> Threeaddrassem.format three_addr_assem_filtered);
     *)
