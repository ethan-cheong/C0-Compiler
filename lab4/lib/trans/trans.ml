 ...
 *)
let translate_ast (ast : A.mprogram_block) defined_functions env struct_to_size : T.func =
  (*
    ...
    let rec trans_exp ?(call_as_stm = false) ?(address_of = false) (exp : A.exp) =
      ...
      *)
      | Alloc_array { array_type; size } ->
        ...
        Temp t2
      (* Array access with "nice" scale (1,2,4 or 8)
      ...
      *)
      | Array_access { array; index; array_type }
        ...
        then (
          ...
          Temp t3)
      (* 
          Struct/irregular shaped array access:
          ([
        ins(arr_exp);
        a <- res(arr_exp); 
        ins (index_exp);
      ...
      | Array_access { array; index; array_type } ->
        ...
        (match address_of with
         ...
         | false ->
           ...
           Temp t6)
        (* Struct access when offset is less than 2^32 max value:
        tr(struct_access(s, offset)) -> 
         ([
        a <- &s;  // quad
...
let translate (ast_list : Ast.program) struct_to_size : T.program =
  ...
  List.map ast_list_only_defs ~f:(fun ast ->
    translate_ast ast defined_functions env struct_to_size)
;;
