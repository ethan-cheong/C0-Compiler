...
let rsp = AS.Reg { reg = SP; size = QUAD }
...
let add_callee_saved_reg ~reg_counter operand =
  ...
  | AS.Reg { reg; _ } ->
    let r =
      ...
      | _ -> -1
    in
    if r >= 0 then Hashtbl.set reg_counter ~key:r ~data:operand
  | _ -> ()
;;
...
(** [calculate_offsets body main] returns a tuple [(stack_offset, reg_counter)].

    [stack_offset] is the required stack offset for our function.
    [reg_counter] is a hashtbl containing the callee-saved registers. If 
    main, then this is empty.
    *)
let calculate_offsets (body : AS.instr list) (main : bool) =
  ...
  let extract_temps_and_regs (instr : AS.instr) =
    ...
    | Unop { src; _ } ->
      ...
      if not main then add_callee_saved_reg ~reg_counter src
    | Binop { src; dest; _ }
    | Mov { dest; src }
    | Movsx { dest; src }
    | Movzx { dest; src }
    | Mov_trunc { dest; src } ->
      ...
      if not main then add_callee_saved_reg ~reg_counter dest
    | Load { base; index; dest; _ } ->
      add_temp ~temp_counter base;
      if not main then add_callee_saved_reg ~reg_counter base;
      add_temp ~temp_counter index;
      if not main then add_callee_saved_reg ~reg_counter index;
      add_temp ~temp_counter dest;
      if not main then add_callee_saved_reg ~reg_counter dest
    ...
    | Test { lhs; rhs } | Cmp { lhs; rhs } ->
      ...
      if not main then add_callee_saved_reg ~reg_counter lhs;
      add_temp ~temp_counter rhs;
      if not main then add_callee_saved_reg ~reg_counter rhs
    | _ -> ()
  in
  ...
  let stack_offset =
    Int32.of_int_exn
      ...
      ((Hashtbl.length temp_counter + Hashtbl.length reg_counter + 1) * 8)
    (* Before lab4, this 1 wasn't there. 
       Increment by one extra so that the return address isn't overwritten by a movq. *)
  in
...
let stack_spill_func
  ...
  temp_table
  stack_offset
  (reg_counter : (int, AS.operand) Hashtbl.t)
  can_use_red_zone
  : AS.instr list
  =
  ...
  in
  (* Replaces the temps in the mov instruction. Don't bother with 
    scratch register usage yet - we deal with that in codegen.
      *)
  let spill_temps_mov (dest : AS.operand) (src : AS.operand) (mov_type : mov_type)
    : AS.instr
    =
    AS.(
      let dest_part =
        ...
        | Temp t ->
          ...
          Stack_ref { offset = dest_offset; size = Temp.size t }
        | _ -> dest
      in
      let src_part =
        match src with
        | Temp t ->
          ...
          Stack_ref { offset = src_offset; size = Temp.size t }
        | _ -> src
      in
      match mov_type with
      | Mov -> AS.Mov { dest = dest_part; src = src_part }
      | Movsx -> AS.Movsx { dest = dest_part; src = src_part }
      | Movzx -> AS.Movzx { dest = dest_part; src = src_part }
      | Mov_trunc -> AS.Mov_trunc { dest = dest_part; src = src_part })
  in
  ...
  let spill_temps_binop binop : AS.instr =
    ...
    | AS.Binop { op; src; dest } ->
      (match src with
       | Temp t ->
         AS.Binop
           { src = Stack_ref { offset = ref_temp_from_table t; size = Temp.size t }
           ; dest
           ; op
           }
       | _ -> binop)
  ...
  let spill_temps_cmp (lhs : AS.operand) (rhs : AS.operand) =
    ...
    | _, Temp t ->
      Cmp { lhs; rhs = Stack_ref { offset = ref_temp_from_table t; size = Temp.size t } }
    | _ -> Cmp { lhs; rhs }
  in
  let spill_temps_load disp base index scale dest =
    AS.(
      let dest_part =
        match dest with
        | Temp t ->
      ...
      let base_part =
        match base with
        | Temp t ->
          let base_offset = ref_temp_from_table t in
          Stack_ref { offset = base_offset; size = Temp.size t }
        | _ -> base
      in
      let index_part =
        match index with
        | Temp t ->
          let index_offset = ref_temp_from_table t in
          Stack_ref { offset = index_offset; size = Temp.size t }
        | _ -> index
      in
      Load { disp; base = base_part; index = index_part; scale; dest = dest_part })
  in
  let spill_temps_lea disp base index scale dest =
    AS.(
      let dest_part =
        match dest with
        | Temp t ->
      ...
      let index_part =
        ...
        | _ -> index
      in
      Lea { disp; base = base_part; index = index_part; scale; dest = dest_part })
  in
  let spill_temps_store disp base index scale src =
    AS.(
      let src_part =
        match src with
        | Temp t ->
          let src_offset = add_temp_to_table t in
          Stack_ref { offset = src_offset; size = Temp.size t }
        | _ -> src
      in
      let base_part =
      ...
      let index_part =
        ...
        | _ -> index
      in
      Store { disp; base = base_part; index = index_part; scale; src = src_part })
  in
  let spill_temps_ret () =
    let epilogue =
      if can_use_red_zone || Int32.(stack_offset = zero)
      then AS.[ Ret ]
      else AS.[ Ret; Binop { op = Add; src = Imm stack_offset; dest = rsp } ]
    in
      ...
      Hashtbl.fold reg_counter ~init:[] ~f:(fun ~key:_ ~data:r acc ->
        ...
        | Reg { reg; _ } ->
          AS.Mov
            ...
            ; dest = Reg { reg; size = QUAD } (* Always restore the full quad*)
            }
          :: acc
        | _ -> failwith "Somehow reg counter contained non regs")
    in
  ...
  let translate_call (call : AS.instr) =
    ...
    | Call { ident; args } ->
      ...
      @ List.rev_mapi args ~f:(fun i arg ->
          ...
          | Temp t ->
            AS.Mov
              { dest = map_index_to_operand i (Temp.size t)
              ; src =
                  Stack_ref
                    { offset = Int32.(ref_temp_from_table t + stack_decr)
                    ; size = Temp.size t
                    }
              }
          | _ ->
            AS.Mov { dest = map_index_to_operand i (AS.size_of_operand arg); src = arg })
  ...
  let rec stack_spill_func_helper (lst : AS.instr list) (acc : AS.instr list)
    ...
    | [] -> acc
    | Mov { dest; src } :: t ->
      stack_spill_func_helper t (spill_temps_mov dest src Mov :: acc)
    | Movsx { dest; src } :: t ->
      stack_spill_func_helper t (spill_temps_mov dest src Movsx :: acc)
    | Movzx { dest; src } :: t ->
      stack_spill_func_helper t (spill_temps_mov dest src Movzx :: acc)
    | Mov_trunc { dest; src } :: t ->
      stack_spill_func_helper t (spill_temps_mov dest src Mov_trunc :: acc)
    | Load { disp; base; index; scale; dest } :: t ->
      stack_spill_func_helper t (spill_temps_load disp base index scale dest :: acc)
    | Store { disp; base; index; scale; src } :: t ->
      stack_spill_func_helper t (spill_temps_store disp base index scale src :: acc)
    | Lea { disp; base; index; scale; dest } :: t ->
      stack_spill_func_helper t (spill_temps_lea disp base index scale dest :: acc)
  ...
  let move_stack_section =
    if can_use_red_zone || Int32.(stack_offset = zero)
    then []
    else [ AS.Binop { op = Sub; src = Imm stack_offset; dest = rsp } ]
  in
  let prologue =
    ...
    Hashtbl.fold reg_counter ~init:[] ~f:(fun ~key:_ ~data:r acc ->
      ...
      | Reg { reg; _ } ->
        AS.Mov
          { src = Reg { reg; size = QUAD } (* Always save the full quad.*)
          ; dest =
              Stack_ref
                { size = QUAD
                ; offset =
        ...
        :: acc
      | _ -> failwith "Somehow a non-reg ended up in the reg counter")
    @ move_stack_section
  in
...
let adjust_stack_offsets (lst : AS.instr list) (adj : Int32.t) : AS.instr list =
  ...
  List.map lst ~f:(fun instr ->
    ...
    | Movsx { src; dest } -> Movsx { src = adjust_ref src; dest = adjust_ref dest }
    | Movzx { src; dest } -> Movzx { src = adjust_ref src; dest = adjust_ref dest }
    | Mov_trunc { src; dest } ->
      Mov_trunc { src = adjust_ref src; dest = adjust_ref dest }
    | Load { disp; base; index; scale; dest } ->
      Load
        { disp
        ; base = adjust_ref base
        ; index = adjust_ref index
        ; scale
        ; dest = adjust_ref dest
        }
...
let stack_spill (lst : AS.func list) ~only_main : AS.func list =
  ...
  let stack_spill_helper (func : AS.func) temp_table =
    ...
    | true ->
      ...
      let main = true in
      let (stack_offset : Int32.t), reg_counter = calculate_offsets body main in
      let translated_body =
    ...
    | false ->
      let signature, body = func in
      let main = String.(signature.label = "_c0_main") in
      let (stack_offset : Int32.t), reg_counter = calculate_offsets body main in
