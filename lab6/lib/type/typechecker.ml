...
(* 
   Context now probably changes? 
    Vars do not have to be initialised in syn_exp if in anno
     Now, in_anno to check if it is checking an annotation
     in_defn to check if in defn?
     in_loop to check if it is loop?
...
let rec extract_tau ~ctx ?(function_call = false) ?(req_struct_defined = false) tau =
  ...
  | Void ->
    if function_call
    then Void
    else Void (* Void might be used now, but must only be as a pointer *)
  | Alias s ->
...
(* Need to compare  *)
let rec compare_tau ~ctx ?(is_ret_type = false) ?(is_void_pointer = false) t1 t2 : A.tau =
  match t1, t2 with
  | A.Int, A.Int -> A.Int
  | A.String, A.String -> A.String
  | A.Char, A.Char -> A.Char
  | A.Bool, A.Bool -> A.Bool
  | A.Pointer t1', A.Pointer t2' ->
    A.Pointer (compare_tau ~ctx ~is_void_pointer:true t1' t2')
  | A.Struct s1, A.Struct s2 ->
    if String.compare (Symbol.name s1) (Symbol.name s2) <> 0
    then
      failwith
        (sprintf "struct %s and %s are incompatible" (Symbol.name s1) (Symbol.name s2));
    A.Struct s1
  | A.Array t1', A.Array t2' -> A.Array (compare_tau ~ctx t1' t2')
  (* Type subsumption rule *)
  | A.Any, A.Pointer p -> A.Pointer (extract_tau ~ctx p)
  | A.Pointer p, A.Any -> A.Pointer (extract_tau ~ctx p)
  | A.Any, A.Any -> A.Any
  | A.Void, A.Void ->
    (* Correct? *)
    if is_void_pointer || is_ret_type
    then A.Void
    else failwith "void, void received but not a return type or used in a pointer"
  | _, _ ->
    failwith
      (sprintf
...
let validate_lvalue exp : bool =
  A.(
    ...
    | Cast _ -> failwith "casts are not a valid lvalue"
    | Struct_access _ -> failwith "no struct access elab yet"
    | Anno_result -> failwith "anno result not an lvalue"
    | Anno_index -> failwith "anno_index not an lvalue"
    | Anno_elem _ -> failwith "anno_elem not an lvalue"
    | Forall _ -> failwith "forall not an lvalue"
    | Exists _ -> failwith "exists not an lvalue"
    | String_const _ -> failwith "string const not an lvalue"
    | Anno_length _ -> failwith "anno length not an lvalue")
;;
...
let rec extract_lvalue_symbol exp : Symbol.t =
  A.(
    ...
    | Cast { operand; _ } -> extract_lvalue_symbol (Mark.data operand)
...
let assert_small_type tau =
  A.(
    match tau with
    | Void | Int | Bool | Pointer _ | Array _ | Any | Char | String -> ()
...
let assert_not_void tau =
  A.(
    match tau with
    | Void -> failwith "void not valid here"
    | _ -> ())
;;
...
let assert_bool tau =
  A.(
    match tau with
    | Bool -> ()
    | _ -> failwith "only bools allowed")
;;

let assert_not_string tau =
  A.(
    match tau with
    | String -> failwith "string not valid here"
    | _ -> ())
;;
...
let strip_type tau =
  A.(
    match tau with
    | Pointer x -> x
    | Any -> Void (* Might have this case for NULL*)
    | _ -> failwith "s_type is a pointer")
;;
...
let rec syn_exp (ctx : env) ?(initializing = false) (mexp : A.mexp) : A.tau * A.mexp =
  ...
  in
  let anno_state = ctx.anno_state in
  ...
  | A.Var { ident; _ } ->
    ...
    (match search with
     | None -> error ~msg:(sprintf "Var %s has no associated type hi" (Symbol.name ident))
     | Some x ->
       if not initializing
       then (
  ...
  | A.Comparison { op; lhs; rhs } ->
    ...
    (match op with
     ...
     | Equal | Not_equal ->
       ...
       assert_not_void binop_tau;
       assert_not_string binop_tau;
  ...
  | A.Function_call { ident; args; _ } ->
    ...
    let params, fn_tau =
      ...
      | None ->
        failwith (sprintf "function not declared before use: %s\n" (Symbol.name ident))
    in
  ...
  | Cast { pointer_type; operand; _ } ->
    let s_type, s_exp = syn_exp ctx operand in
    assert_pointer s_type;
    assert_pointer pointer_type;
    let stripped_type = strip_type s_type in
    let stripped_pointer = strip_type pointer_type in
    if is_void_pointer pointer_type && not (is_void_pointer s_type)
    then
      ( Pointer Void
      , Mark.mark
          (A.Cast
             { pointer_type = stripped_pointer
             ; operand = s_exp
             ; orig_type = stripped_type
             })
          span )
    else (
      (* s_type needs to be of void_pointer *)
      assert (is_void_pointer s_type);
      ( pointer_type
      , Mark.mark
          (A.Cast
             { pointer_type = stripped_pointer
          ...
          span ))
  | Anno_result ->
    if not anno_state.in_ensures then failwith "anno_result not in ensures";
    anno_state.ret_type, Mark.mark Ast.Anno_result span
  | Anno_length mexp ->
    if (not anno_state.in_stmt)
       && (not anno_state.in_header)
       && (not anno_state.in_ensures)
       && not anno_state.in_loop
    then failwith "anno_length not in any annotation";
    let s_type, s_exp = syn_exp ctx mexp in
    (match s_type with
     | Array _ -> Int, Mark.mark (Ast.Anno_length s_exp) span
     | _ -> failwith "length not called on an array")
  | String_const _ -> String, mexp
  | Anno_elem _ ->
    if (not anno_state.in_stmt)
       && (not anno_state.in_header)
       && (not anno_state.in_ensures)
       && not anno_state.in_loop
    then failwith "anno_elem not in any annotation";
    let elem_type = ctx.anno_state.arr_type in
    (match elem_type with
     | Ast.Dummy -> failwith "elem type cant be dummy"
     | _ -> ());
    elem_type, Mark.mark (Ast.Anno_elem elem_type) span
  | Anno_index ->
    if (not anno_state.in_stmt)
       && (not anno_state.in_header)
       && (not anno_state.in_ensures)
  ...
  | Forall { arr; condition } ->
    let arr_type, arr_exp = syn_exp ctx arr in
    let elem_type =
      match arr_type with
      | Array s -> s
      | _ -> failwith "arr_type must be arr"
    in
    let condition_ctx =
      { ctx with anno_state = { ctx.anno_state with arr_type = elem_type } }
    in
    let condition_type, condition_exp = syn_exp condition_ctx condition in
    assert_bool condition_type;
    Ast.Bool, Mark.mark (Ast.Forall { arr = arr_exp; condition = condition_exp }) span
  | Exists { arr; condition } ->
    ...
    let condition_type, condition_exp = syn_exp condition_ctx condition in
    assert_bool condition_type;
    Ast.Bool, Mark.mark (Ast.Exists { arr = arr_exp; condition = condition_exp }) span

and check_spec ctx spec =
  let anno_state = ctx.anno_state in
  Ast.(
    match spec with
    | Requires mexp ->
      if not anno_state.in_header then failwith "requires not in header";
      let res_tau, new_exp = syn_exp ctx mexp in
      assert_bool res_tau;
      Requires new_exp
    | Ensures mexp ->
      if not anno_state.in_header then failwith "ensures not in header";
      let res_tau, new_exp =
        syn_exp { ctx with anno_state = { ctx.anno_state with in_ensures = true } } mexp
      in
      assert_bool res_tau;
      Ensures new_exp
    | Loop_invariant mexp ->
      if not anno_state.in_loop then failwith "loop invariant not in loop";
      let res_tau, new_exp = syn_exp ctx mexp in
      assert_bool res_tau;
      Loop_invariant new_exp
    | Assert_spec mexp ->
      if (not anno_state.in_stmt)
         || anno_state.in_ensures
         || anno_state.in_header
         || anno_state.in_loop
      then failwith "assert spec not in a statement";
      let res_tau, new_exp = syn_exp ctx mexp in
      assert_bool res_tau;
      Assert_spec new_exp)

(* todo: if pointer_type is void, then ch *)
...
let rec chk_stm (ctx : env) (mstm : A.mstm) (tau : A.tau) : env * A.mstm =
        ...
        | None ->
          let new_env, new_s =
            chk_stm
              ...
              ; anno_state = { ctx.anno_state with in_loop = false }
              }
              s
              tau
          in
  ...
  | While (e, s) ->
    ...
    (match exp_type with
     | A.Bool ->
       let _, validated_stm =
         chk_stm { ctx with anno_state = { ctx.anno_state with in_loop = true } } s tau
       in
  ...
  | Asnop_pure_mem { lhs; op; rhs } ->
    ...
    (match search with
     | None ->
       error ~msg:(sprintf "Var %s has no associated type hi2" (Symbol.name lhs_symbol))
     | Some _ -> ());
    let (_ : unit) =
  ...
  | Asnop_impure_mem { lhs; op; rhs } ->
    ...
    (match search with
     | None ->
       error ~msg:(sprintf "Var %s has no associated type hi3" (Symbol.name lhs_symbol))
     | Some _ -> ());
    let (_ : unit) =
      match SymbolSet.find ctx.delta ~f:(match_symbol lhs_symbol) with
      | None ->
  ...
  | Annotated_stm_intermediate _ ->
    failwith "annotated_stm_intermediate to be elaborated out"
  | Annotated_stm (anno, mstm) ->
    let new_anno =
      List.map anno ~f:(fun spec ->
        check_spec { ctx with anno_state = { ctx.anno_state with in_stmt = true } } spec)
    in
    let ctx', validated_stmt =
      chk_stm { ctx with anno_state = { ctx.anno_state with in_loop = false } } mstm tau
    in
    ctx', Mark.mark (Ast.Annotated_stm (new_anno, validated_stmt)) span
;;
...
let typecheck_fndef_anno ctx ~ret_type ~param_list ~fn_block ~anno : env * A.mstm * A.anno
  =
  (* All the args have to be added into the context like a declaration *)
  let curr_ctx =
    { gamma = SymbolMap.empty
  ...
  let new_anno =
    List.map anno ~f:(fun spec ->
      check_spec
        { curr_ctx' with anno_state = { curr_ctx'.anno_state with in_header = true } }
        spec)
  in
  let new_ctx, new_fn_block = chk_stm curr_ctx' fn_block ret_type in
  let validated_tau = extract_tau ~ctx:new_ctx ~function_call:true ret_type in
  ...
  | true, A.Array _
  | true, A.Any
  | false, A.Void -> new_ctx, new_fn_block, new_anno
  | _ ->
    failwith
      (sprintf
         "ret_type %s does not align with function return %s"
...
let rec check_program (ctx : env) (program : A.program_block) : env * A.program_block =
  ...
  | Function_Def_Anno { ret_type; ident; params; anno; fn_block } ->
    check_fndef_anno ctx ~ret_type ~ident ~params ~fn_block ~anno
  ...
  | Function_Def_Anno_Intermediate _ ->
    failwith "fn_defn_anno_intermediate not allowed after elaboration"
...
and check_header (ctx : env) (program : A.program_block) : env * A.program_block =
  ...
  | Function_Def_Anno _ -> failwith "fn_defn_anno not allowed in headers"
  ...
  | Function_Def_Anno_Intermediate _ ->
    failwith "fn_defn_anno_intermediate not allowed in headers"
...
and check_fndef (ctx : env) ~ret_type ~ident ~params ~fn_block =
  ...
  | Some (_, _, defined) ->
    ...
    let validated_tau = valid_fn_type ctx ~ret_type in
    let new_ctx =
      { ctx with anno_state = { ctx.anno_state with ret_type = validated_tau } }
    in
    let ret_ctx, updated_fn_block =
      typecheck_fndef new_ctx ~ret_type ~param_list ~fn_block
    in
    let (_ : unit) =
...
and check_fndef_anno (ctx : env) ~ret_type ~ident ~params ~fn_block ~anno =
  (* 
          1. Assume that the declaration typechecks (since it is elaborated out)
          1a. Means can ignore the checks that we do for function declaration
          2. Make sure that the function has not yet been defined
  ...
  | Some (_, _, defined) ->
    ...
    let new_ctx =
      { ctx with anno_state = { ctx.anno_state with ret_type = validated_tau } }
    in
    let ret_ctx, updated_fn_block, new_anno =
      typecheck_fndef_anno new_ctx ~ret_type ~param_list ~fn_block ~anno
    in
    let (_ : unit) =
      SymbolTable.update ret_ctx.epsilon ident ~f:(fun some_key ->
        match some_key with
