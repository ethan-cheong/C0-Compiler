...
let _tail_call = false

(* Convert eligible functions to tail-recursive with accumulators *)
let _basic_accumulation = false

(* Do function inlining *)
let _function_inlining = false
...
let _ssa = false
let _deadcode_elim = false
let _constant_prop = false
let _sccp = false
let _strength_reduction = false
...
let cmd_line_term : cmd_line_args Cmdliner.Term.t =
  ...
  and remove_annotations =
    let doc = "If present, only typecheck annotations without compiling them." in
    flag (Arg.info [ "d"; "remove-annotations" ] ~doc)
  in
  { verbose
  ; dump_all
  ; dump_parsing
...
let compile (cmd : cmd_line_args) : unit =
  ...
  say_if cmd.dump_all (fun () -> Ast.Print.pp_program ast);
  if cmd.dump_all || cmd.dump_only_times
  ...
  say_if cmd.dump_all (fun () -> Ast.Print.pp_program typechecked_ast);
  ...
  (* Translate *)
  (* TODO: Uncomment this when 3as works. *)
  ...
  (* let struct_to_size = Hashtbl.create ~growth_allowed:true ~size:10 (module Symbol) in *)
  let ir =
    Trans.translate
      typechecked_elab_ast
      struct_to_size
      ~remove_annotations:cmd.remove_annotations
  in
  let only_main = List.length ir = 1 in
  (* TODO: Uncomment this when it becomes necessary *)
  if cmd.dump_all || cmd.dump_only_times
  then Timestamp.mark_timestamp timestamps "translation";
  ...
  (* Optimizations are commented out *)
  (* 
  ...
  *)
  ...
  (* Comment out after instr sel works *)
  ...
  if cmd.dump_all || cmd.dump_only_times
  then Timestamp.mark_timestamp timestamps "codegen";
  say_if cmd.dump_assem (fun () -> Assem.format_program x86_assem);
  say_if cmd.dump_all (fun () -> Assem.format_program x86_assem);
  ...
  | X86_64 ->
    ...
    Out_channel.with_file file ~f:(fun out ->
      ...
      Out_channel.fprintf out "%s\n" (Assem.format_program x86_assem))
;;
