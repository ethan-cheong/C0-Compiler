...
let rec size_of_exp : T.exp -> Temp.size = function
  | Const (Byte _) -> BYTE
  | Const (Int _) | Const MAX_INT -> DOUBLE
  | Const (String _) -> QUAD
  | Temp t -> Temp.size t
...
let rec munch_exp (dest : ThreeAS.operand) (exp : T.exp) (rev_acc : ThreeAS.instr list)
  ...
  | T.Const n ->
    (match n with
     ...
     | String s -> ThreeAS.Mov { dest; src = ThreeAS.String s } :: rev_acc
     | Byte b -> ThreeAS.Mov { dest; src = ThreeAS.Byte_imm b } :: rev_acc)
  | T.Temp t ->
  ...
  *)
  | If { condition; lt; lf } :: tl ->
    (match condition with
     | Const i ->
       (match i with
        | MAX_INT -> failwith "MAX_INT should not be in the condition of an if statement"
        | String _ -> failwith "A string cannot be in the condition of an if statement"
        | Int i ->
          ...
          then ThreeAS.Jump (munch_label lf) :: munch_cmd tl rev_acc
            ...
            failwith
              (Printf.sprintf
                 "How interesting... somehow %d ended up in the condition of an if \
                  statement"
                 (Int32.to_int_exn i))
        | Byte _ -> failwith "TODO: move the logic here when u do bools")
     ...
     | Temp dest_temp, Temp base_temp, Const (Int i) ->
       munch_cmd
         ...
         (ThreeAS.Lea
          ...
          :: rev_acc)
     | _ -> failwith "Invalid fields in memory_read in intcodegen.. messed up in trans.")
  | Nop :: tl -> munch_cmd tl (Nop :: rev_acc)
;;
...
let munch_func (func : T.func) : ThreeAS.func =
  ...
  | { ident; args; tail_rec; num_temps; recursive } ->
