...
let convert_op = function
  ...
  | ThreeAS.String s -> AS.String s
  | ThreeAS.Byte_imm b -> AS.Byte_imm b
;;
...
let size_of_op = function
  ...
  | ThreeAS.String _ -> Temp.QUAD
  | ThreeAS.Byte_imm _ -> Temp.BYTE
;;
  ...
  ThreeAS.(
    ...
    | _ ->
      ...
      then
        failwith
          (Printf.sprintf
             "operand size mismatch in instrsel %s %s %s binop"
             (ThreeAS.format_operand dest)
             (ThreeAS.format_operand lhs)
             (ThreeAS.format_operand rhs));
...
(**
    ...
    However, if either lhs or rhs is a byte (in which case both are bytes), 
...
let translate_binop
  ...
  | ThreeAS.Shr -> translate_shift dest lhs rhs AS.Shr label_number caller_name
;;
