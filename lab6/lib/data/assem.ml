...
module StringTable = Type_modules.StringTable
...
type operand =
  ...
  | String of string (* This should not be found in final assem!*)
  | Label_op of string (* This can be found in the final assem *)
[@@deriving compare, sexp, hash, equal]
...
type program = string StringTable.t * func list
...
let format_operand = function
  | Imm imm | Byte_imm imm -> "$" ^ Int32.to_string imm
  | MAX_INT -> "$0xffffffff"
  | Temp t -> Temp.name t
...
let format_operand_with_new_size operand new_size =
  ...
  | String _
  | Label_op _
  | Byte_imm _ -> format_operand operand
...
let format = function
  ...
  | Mov { src; dest } ->
    (match src, dest with
     (* If the destination is a stack ref, and the src is a BYTE, we have to use
       movzbl. *)
     | Stack_ref { size = BYTE; _ }, _ ->
       sprintf
         "movzbl\t %s, %s\n"
         (format_operand src)
...
let format_program program =
  let string_table = fst program in
  let body = snd program in
  let prologue =
    StringTable.fold string_table ~init:"" ~f:(fun ~key:s ~data:label acc ->
      Printf.sprintf "%s:\n\t.string \"%s\"\n" label s ^ acc)
  in
  prologue
  ^ List.fold (List.map body ~f:format_func) ~init:"" ~f:(fun x y -> x ^ "\n" ^ y)
;;
