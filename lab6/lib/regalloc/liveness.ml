...
let used_nodes_call ~(args : AS.operand list) : NodeSet.t =
  ...
  let used_vals =
    List.fold_left args ~init:[] ~f:(fun acc arg ->
      ...
      | AS.MAX_INT | AS.Imm _ | AS.Addr_imm _ | String _ | Byte_imm _ -> acc
      | AS.Stack_ref _ -> failwith "no refs allowed when extracting args"
      | Label_op _ -> failwith "Label operands should not be found before codegen")
  in
...
let node_of_operand_opt (op : AS.operand) : Node.t option =
  match op with
  | MAX_INT | Imm _ | Addr_imm _ | String _ | Byte_imm _ -> None
  | Label_op _ -> failwith "Label operands should not be found before codegen"
